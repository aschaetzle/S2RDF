...
object QueryExecutor {
  ...
  private def getPath(tType:String, tPath: String): String = {
    ...
    if (tType=="VP"){
      // HACK for ExtVPBigTable redirect VP to ExtVP_BigTable directory
      res = Settings.databaseDir+"ExtVP_BigTable/"+tPath.replace("/","")+".parquet";
  ...
  private def preloadTables(qr:Query, actualPostfix:String) = {
    ...
    for (tableStat <- qr.tables.values){
      ...
      if (_cachedTables contains fileName){
        ...
        if (actualPostfix.contains("SO") && Settings.partNumberExtVP != 432)
          table = _sqlContext.parquetFile(fileName)
                  .repartition(Settings.partNumberExtVP);
        else if (Settings.partNumberVP != 432)
          table = _sqlContext.parquetFile(fileName)
  ...
  def runTests() = {   
    ...
    var testSet = ""    
    for (queryN <- queryNames) if (queryN.contains("IL10")){      
