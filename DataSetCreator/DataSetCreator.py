...
# Delay in seconds for waiting until cluster is recovered after execution
...
def submitSparkCommand(relationType):
    ...
    #command = "spark-submit --num-executors 9 --executor-memory 16g --class firstRun --master yarn "
    
    # generate submit command // yarn-master
    #command = ("spark-submit --num-executors 9 --executor-memory 16g --driver-memory "
    #+ driverMemory
    #+ " --class runDriver --master yarn "
    #+ jarFilePath + " "
    #+ dbDir + " " + inpuRDFFile + " " + relationType + " " + "{:.1f}".format(scaleUB)    
    #+ " > ./DataBaseCreator" + relationType + ".err")

    # generate submit command // spark-master
