...
public class SparkTableStatistics {
	...
	public static String generateTablesUsageInstructions(){
		...
		for (String placeHolderName:tableCandidatesMap.keySet()){
			...
			// HACK for ExtVP_BigTable. Pass only first table of the relation to ensure that we
			// take ExtVP_BigTable instead ExtVPTable at the QueryExecutor
			//res+=placeHolderName+"\t"+candList.bestCandidateID+"\t"+bestCandidate[0]+ "\t" + bestCandidate[1]+ "/" + bestCandidate[2]+"\n";
			res+=placeHolderName+"\t"+candList.bestCandidateID+"\t"+"VP"+ "\t" + bestCandidate[1]+"\n";
	...
	/**
	 * Adds corresponding TP relation to every Statement as where condition
	 * For Example: TP Relation: SS pred1 pred2 -> where (pred1 table).pred2 = true
	 * @param statements the list of sql-statements
     */
	public static void addTPRelationsToStatements(ArrayList<SqlStatement> statements){
		for (SqlStatement stat: statements){
			String tabName = stat.getFrom();
			for (String[] rel: tableCandidatesMap.get(tabName).getSqlTableCandidates()){
				if (!rel[0].equals("VP")){
					stat.addConjunction(rel[0]+rel[2]+"=true");
				}
			}
		}
	}
