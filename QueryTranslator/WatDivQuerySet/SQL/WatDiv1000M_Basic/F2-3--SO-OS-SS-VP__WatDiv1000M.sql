SELECT tab0.v1 AS v1 , tab7.v0 AS v0 , tab6.v7 AS v7 , tab4.v5 AS v5 , tab5.v6 AS v6 , tab3.v4 AS v4 , tab2.v3 AS v3 , tab1.v2 AS v2 
 FROM    (SELECT sub AS v0 
	 ...
	 WHERE obj = 'wsdbm:SubGenre140' AND SSfoaf__homepage=true AND SSog__title=true AND SSrdf__type=true AND SSsorg__caption=true AND SSsorg__description=true
	) tab7
 JOIN    (SELECT obj AS v1 , sub AS v0 
	 FROM foaf__homepage$$1$$
	 
	 WHERE SSog__title=true AND SSrdf__type=true AND SSsorg__caption=true AND SSsorg__description=true AND OSsorg__url=true AND OSwsdbm__hits=true AND SSwsdbm__hasGenre=true
	) tab0
 ON(tab7.v0=tab0.v0)
 JOIN    (SELECT sub AS v0 , obj AS v4 
	 FROM sorg__caption$$4$$
	 
	 WHERE SSfoaf__homepage=true AND SSog__title=true AND SSrdf__type=true AND SSsorg__description=true AND SSwsdbm__hasGenre=true
	) tab3
 ON(tab0.v0=tab3.v0)
 JOIN    (SELECT sub AS v0 , obj AS v5 
	 FROM sorg__description$$5$$
	 
	 WHERE SSfoaf__homepage=true AND SSog__title=true AND SSrdf__type=true AND SSsorg__caption=true AND SSwsdbm__hasGenre=true
	) tab4
 ON(tab3.v0=tab4.v0)
 JOIN    (SELECT sub AS v0 , obj AS v2 
	 FROM og__title$$2$$ 
	 WHERE SSfoaf__homepage=true AND SSrdf__type=true AND SSsorg__caption=true AND SSsorg__description=true AND SSwsdbm__hasGenre=true
	) tab1
 ON(tab4.v0=tab1.v0)
 JOIN    (SELECT sub AS v0 , obj AS v3 
	 FROM rdf__type$$3$$ 
	 WHERE SSfoaf__homepage=true AND SSog__title=true AND SSsorg__caption=true AND SSsorg__description=true AND SSwsdbm__hasGenre=true
	) tab2
 ON(tab1.v0=tab2.v0)
 JOIN    (SELECT sub AS v1 , obj AS v6 
	 FROM sorg__url$$6$$ 
	 WHERE SOfoaf__homepage=true AND SSwsdbm__hits=true
	) tab5
 ON(tab0.v1=tab5.v1)
 JOIN    (SELECT sub AS v1 , obj AS v7 
	 FROM wsdbm__hits$$7$$ 
	 WHERE SOfoaf__homepage=true AND SSsorg__url=true
	) tab6
 ON(tab5.v1=tab6.v1)
...
------
sorg__description$$5$$	4	VP	sorg__description
...
------
wsdbm__hasGenre$$8$$	4	VP	wsdbm__hasGenre
...
------
sorg__caption$$4$$	1	VP	sorg__caption
...
------
foaf__homepage$$1$$	3	VP	foaf__homepage
