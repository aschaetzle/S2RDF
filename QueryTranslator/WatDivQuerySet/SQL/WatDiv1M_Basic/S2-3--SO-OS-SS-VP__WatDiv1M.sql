SELECT tab0.v1 AS v1 , tab1.v0 AS v0 , tab2.v3 AS v3 
 FROM    (SELECT sub AS v0 
	 ...
	 WHERE obj = 'wsdbm:Country24' AND SSdc__Location=true AND SSwsdbm__gender=true AND SSrdf__type=true
	) tab1
 JOIN    (SELECT sub AS v0 
	 FROM rdf__type$$4$$ 
	 WHERE obj = 'wsdbm:Role2' AND SSdc__Location=true AND SSsorg__nationality=true AND SSwsdbm__gender=true
	) tab3
 ON(tab1.v0=tab3.v0)
 JOIN    (SELECT obj AS v1 , sub AS v0 
	 FROM dc__Location$$1$$ 
	 WHERE SSsorg__nationality=true AND SSwsdbm__gender=true AND SSrdf__type=true
	) tab0
 ON(tab3.v0=tab0.v0)
 JOIN    (SELECT sub AS v0 , obj AS v3 
	 FROM wsdbm__gender$$3$$
	 
	 WHERE SSdc__Location=true AND SSsorg__nationality=true AND SSrdf__type=true
	) tab2
 ON(tab0.v0=tab2.v0)


++++++Tables Statistic
sorg__nationality$$2$$	1	VP	sorg__nationality
...
------
wsdbm__gender$$3$$	2	VP	wsdbm__gender
...
------
dc__Location$$1$$	1	VP	dc__Location
